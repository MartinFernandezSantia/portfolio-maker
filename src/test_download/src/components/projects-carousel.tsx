...
function Project({ project }: ProjectProps) {
  ...
  const getImageUrl = (file: File | string) => {
    ...
    return URL.createObjectURL(file); // es File, creamos blob URL
  };
  ...
  return (
    <Card className="overflow-hidden group hover:glow transition-smooth h-full ">
      <div className="relative w-full h-48 md:h-64 lg:h-72 overflow-hidden">
        <Image
          src={getImageUrl(image)} // image es File o string
          alt={title}
          fill
          className="object-cover transition-smooth group-hover:scale-105"
        />
        <div className="absolute inset-0 from-background/80 to-transparent opacity-0 group-hover:opacity-100 transition-smooth" />
